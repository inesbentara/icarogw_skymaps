...
from .conversions import detector2source_jacobian, detector2source, detector2source_jacobian_q, log_10_lambda_eff_to_log_10_Alpha
...
class CBC_vanilla_rate_tgr(object):
    
    def __init__(self,cosmology_wrapper,mass_wrapper,rate_wrapper,tgr_wrapper,spin_wrapper=None,scale_free=False):
        
        self.cw = cosmology_wrapper
        self.mw = mass_wrapper
        self.rw = rate_wrapper
        self.sw = spin_wrapper
        self.tgrw = tgr_wrapper
        self.scale_free = scale_free
        
        if scale_free:
            self.population_parameters =  self.cw.population_parameters+self.mw.population_parameters+self.rw.population_parameters + self.tgrw.population_parameters
        else:
            self.population_parameters =  self.cw.population_parameters+self.mw.population_parameters+self.rw.population_parameters + self.tgrw.population_parameters + ['R0']
            
        event_parameters = ['mass_1', 'mass_2', 'luminosity_distance','log_10_lambda_eff']
        ...
        self.PEs_parameters = event_parameters.copy()
        self.injections_parameters = event_parameters.copy()
        self.injections_parameters.remove('log_10_lambda_eff')
    ...
    def update(self,**kwargs):
        ...
        '''
        self.cw.update(**{key: kwargs[key] for key in self.cw.population_parameters})
        self.cw.cosmology.build_LIV_integrand(self.tgrw.alpha)
        self.mw.update(**{key: kwargs[key] for key in self.mw.population_parameters})
        self.rw.update(**{key: kwargs[key] for key in self.rw.population_parameters})
        self.tgrw.update(**{key: kwargs[key] for key in self.tgrw.population_parameters})
    ...
    def log_rate_PE(self,prior,**kwargs):
        ...
        log10_Alpha=log_10_lambda_eff_to_log_10_Alpha(kwargs['log_10_lambda_eff'],self.cw.cosmology,z)        
        
        # Sum over posterior samples in Eq. 1.1 on the icarogw2.0 document
        log_weights=self.mw.log_pdf(ms1,ms2)+ self.tgrw.log_pdf(log10_Alpha) + self.rw.rate.log_evaluate(z)+log_dVc_dz \
        -xp.log(prior)-xp.log(detector2source_jacobian(z,self.cw.cosmology))-xp.log1p(z)
